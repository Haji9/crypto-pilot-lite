...
function getC(key: string, ttl = 15000) { const e = cache.get(key); if (e && Date.now() < e.expiresAt) return e.data; cache.delete(key); return null }
function setC(key: string, data: any, ttl = 15000) { cache.set(key, { data, expiresAt: Date.now() + ttl }) }
...
app.get('/futures/tickers', async (c) => {
  const ck = 'ftickers'
  ...
  try {
    ...
    const usdtSwaps = raw
      ...
      .map((t: any) => {
        ...
        // volCcy24h is in base currency (BTC, SOL, etc.) — convert to USD
...
app.get('/futures/klines', async (c) => {
  const symbol = (c.req.query('symbol') || 'BTC').toUpperCase()
  const instId = toInstId(symbol)
  const bar = c.req.query('bar') || '1H'
  const limit = c.req.query('limit') || '300'
...
app.get('/futures/depth', async (c) => {
  const symbol = (c.req.query('symbol') || 'BTC').toUpperCase()
  const instId = toInstId(symbol)
  const sz = c.req.query('sz') || '40'
  ...
  try {
    ...
    const result = {
      instId,
      bids: book.bids.map((b: string[]) => ({ price: parseFloat(b[0]), qty: parseFloat(b[1]), orders: parseInt(b[3] || '0') })),
      asks: book.asks.map((a: string[]) => ({ price: parseFloat(a[0]), qty: parseFloat(a[1]), orders: parseInt(a[3] || '0') })),
      ts: book.ts,
    }
    setC(ck, result, 8000)
    return c.json(result)
...
app.get('/futures/funding', async (c) => {
  const symbol = (c.req.query('symbol') || 'BTC').toUpperCase()
  const instId = toInstId(symbol)
  try {
...
// ─── DIAGNOSTIC ENDPOINT ──────────────────────────────────────
// Runs the full pipeline and returns detailed stage-by-stage results

app.get('/futures/diagnose', async (c) => {
  const log: string[] = []
  ...
  try {
    ...
    return c.json({ error: `Stage 1 failed: ${err.message}`, log, stageStats })
  }

  // ── STAGE 2: Select candidates (top 10 for speed) ──
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    tick(`--- [${i+1}/${candidates.length}] ${sym.symbol} (instId: ${sym.instId}) ---`)

    // ── STAGE 3: Fetch klines ──
    let candles1h: any[] = []
    try {
    ...
    tick(`  EMA9 vs EMA21: ${ema9 > ema21 ? 'BULLISH' : ema9 < ema21 ? 'BEARISH' : 'FLAT'}`)
    tick(`  Price vs EMA200: ${closes[closes.length-1] > ema200 ? 'ABOVE' : 'BELOW'}`)
    ...
    else { tick(`  REJECTED: EMAs flat - no direction`); count('rejectedNoDirection'); rejectionReasons['flat_emas'] = (rejectionReasons['flat_emas'] || 0) + 1; continue }
    ...
    tick(`  SCORE: ${score}/117 = ${confidence}% | Direction: ${direction} | Reasons: ${reasons.join(', ')}`)
    count('analyzed')
    count('passed')
  ...
  tick(`\n=== FINAL RESULTS ===`)
  tick(`Tickers loaded: ${stageStats.totalTickers || 0}`)
  tick(`Candidates scanned: ${stageStats.candidates || 0}`)
  tick(`Klines loaded: ${stageStats.klinesLoaded || 0}`)
  tick(`Klines failed: ${stageStats.klinesFailed || 0}`)
  tick(`Candles valid: ${stageStats.candlesValid || 0}`)
  tick(`Fully analyzed: ${stageStats.analyzed || 0}`)
